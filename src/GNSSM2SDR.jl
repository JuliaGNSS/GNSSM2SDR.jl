...
BOC-family signals are replicated sub-chip: the channel's subcarrier table is
evaluated off the modulation GNSSSignals models ([`ReplicaShape`](@ref)) and
programmed alongside the code, and five-tap channels place each tap from the
contract's own `tap_sample_shifts` (GNSSM2SDR.jl#10). This needs gateware with
CSR layout 3 streaming DMA1 record format 2 (gnss-m2sdr#32); an older or newer
build is refused by name at construction.
"""
module GNSSM2SDR
